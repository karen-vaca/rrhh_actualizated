...
requerirAcceso();
  ...
  function mensajeTexto(?string $mensaje): ?string {
      return match ($mensaje) {
          ...
          'validacion' => 'No se guardó el contrato: ' . ($_GET['texto'] ?? 'hay datos no válidos.'),
  ...
  // Sugerencias para "Jefe inmediato" (no hay catálogo de jefes): trabajadores activos
  // y los jefes que ya se han registrado en contratos.
  $sugerenciasJefe = [];
  try {
  ...
  .form-sec-lbl{font-size:var(--tx-etiqueta);letter-spacing:.8px;text-transform:uppercase;color:#008c3b;font-weight:var(--tx-peso-titulo);margin-bottom:12px}
  ...
  .form-input,.form-select,.form-textarea{width:100%;border:1px solid var(--border);border-radius:12px;background:#fff;font-family:'DM Sans',sans-serif;font-size:var(--tx-valor);color:var(--text);outline:0}
  ...
  .wizard-step{flex:1;display:flex;align-items:center;gap:10px;color:var(--text-soft);font-size:12px;font-weight:var(--tx-peso-enfasis);position:relative}
  ...
  .package-head{display:flex;align-items:flex-start;justify-content:space-between;gap:12px;margin-bottom:13px}.package-title{font-family:var(--tx-fuente-titulos);font-size:var(--tx-titulo-seccion);font-weight:var(--tx-peso-titulo);color:var(--tx-color);letter-spacing:-.2px}.package-sub{font-size:12px;color:var(--text-soft);margin-top:4px}.package-pill{display:inline-flex;align-items:center;gap:6px;border-radius:999px;padding:6px 10px;background:#ecfdf3;border:1px solid #baf7cf;color:#128a3d;font-size:var(--tx-insignia);font-weight:var(--tx-peso-enfasis);white-space:nowrap}
  .doc-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:12px}.doc-card{background:#fff;border:1px solid var(--border);border-radius:14px;padding:14px;display:flex;flex-direction:column;gap:9px;min-height:150px}.doc-icon{width:38px;height:38px;border-radius:12px;display:flex;align-items:center;justify-content:center;font-weight:900}.doc-icon.contract{background:#e8f8ed;color:#128a3d}.doc-icon.profile{background:#eff6ff;color:#2563eb}.doc-icon.training{background:#fff7ed;color:#c2410c}.doc-name{font-size:13px;font-weight:var(--tx-peso-enfasis);color:var(--tx-color)}.doc-desc{font-size:11.5px;color:var(--text-soft);line-height:1.35;flex:1}.doc-meta{font-size:var(--tx-etiqueta);color:var(--tx-color-suave);font-weight:var(--tx-peso-normal)}
  ...
  /* Aclaración del salario base (no incluye el auxilio de transporte) */
  .label-aclaracion{font-weight:500;color:var(--text-soft);text-transform:none;letter-spacing:0}
  .ayuda-campo{margin-top:6px;font-size:11.5px;line-height:1.45;color:var(--text-soft)}
  .ayuda-campo strong{color:var(--text-mid)}
  ...
  <div class="layout">
    ...
    <main class="main">
      ...
      $busquedaTopbar = ['placeholder' => 'Buscar contratos, documentos, cargos...', 'id' => 'busquedaGlobal', 'oninput' => 'filtrarTabla()'];
          ...
          <div class="form-section">
            ...
            <div class="form-grid">
              ...
              <div class="form-group">
                ...
                <input class="form-input" type="text" name="jefe_inmediato" id="jefe_inmediato" placeholder="Nombre del jefe" required maxlength="120" list="listaJefes" autocomplete="off">
                <datalist id="listaJefes">
                  <?php foreach ($sugerenciasJefe as $nombreJefe): ?>
                    <option value="<?= e($nombreJefe) ?>"></option>
                  <?php endforeach; ?>
                </datalist>
              </div>
            ...
            <div class="form-grid g3">
              ...
              <div class="form-group">
                ...
                <div class="ayuda-campo" id="ayudaSalarioBase">
                  ...
                  Ej.: con el salario mínimo 2026 aquí se escribe <strong><?= e(fmtMoney(SALARIO_MINIMO_VIGENTE)) ?></strong>, no <?= e(fmtMoney(SALARIO_MINIMO_VIGENTE + AUXILIO_TRANSPORTE_VIGENTE)) ?> (que ya incluye el auxilio).
                </div>
              </div>
...
<div class="wizard-panel" id="wizardStep3">
    ...
    <div class="form-section">
        ...
        <div class="review-grid">
            ...
            <div class="review-card">
                <div class="review-label">Salario base (sin auxilio)</div>
            ...
            <div class="review-card">
                <div class="review-label">Auxilio de transporte</div>
  ...
  // ── Validaciones de campo (mismas reglas que validaciones_contrato.php) ──
  ...
  function errorJefe(v) {
    v = String(v || '').replace(/\s+/g, ' ').trim();
    if (!v) return 'El jefe inmediato es obligatorio.';
    if (v.length < 3 || v.length > 120) return 'El nombre del jefe inmediato debe tener entre 3 y 120 caracteres.';
    if (!/^\p{L}[\p{L} .'-]*[\p{L}.]$/u.test(v)) return 'El nombre del jefe inmediato solo puede contener letras y espacios (sin números ni símbolos).';
    return '';
  }

  // Fecha local (no UTC): toISOString() daba la fecha de mañana desde las 7 p. m. en Colombia.
  function hoyLocal() {
    const d = new Date();
  ...
  function validarPasoContrato(paso) {
    if (paso === 1) {
      ...
      const mensajeJefe = errorJefe($('jefe_inmediato')?.value);
    ...
    if (paso === 2) {
      ...
      const errorFechas = errorFechasContrato();
      marcarErrorCampo('fecha_inicio', errorFechas && errorFechas[0] === 'fecha_inicio' ? errorFechas[1] : '');
      marcarErrorCampo('fecha_fin', errorFechas && errorFechas[0] === 'fecha_fin' ? errorFechas[1] : '');
      if (errorFechas) {
        return focoConAlerta(errorFechas[0], errorFechas[1]);
      }
      ...
      for (const id of ['salario_base_view', 'auxilio_transporte_view']) {
        const campo = $(id);
        const mensaje = campo && campo.validarMonto ? campo.validarMonto() : errorMonto(id, campo?.value);
        if (mensaje) return focoConAlerta(id, mensaje);
      }
    }
  ...
  function mensajeErrorContrato(codigo) {
    const mensajes = {
      datos_incompletos: 'Faltan datos obligatorios. Revisa el formulario.',
      monto_invalido: 'El salario o el auxilio de transporte no son válidos.',
      validacion: 'No se guardó el contrato: hay datos no válidos.',
      id_invalido: 'El ID enviado no es válido.',
      accion_invalida: 'La acción enviada no es válida.',
      error: 'Ocurrió un error al procesar la solicitud.'
    };
    ...
    fetch('guardar.php', {
      ...
      .then(function(data){
        if (data && data.ok && data.id_contrato) {
          ...
          const base = (data && data.mensaje === 'validacion' && data.texto)
...
function actualizarRevisionContrato() {
  ...
  function limpiarFormulario(){
    ...
    if (typeof actualizarAyudaAuxilio === 'function') setTimeout(actualizarAyudaAuxilio, 0);
    setWizardPasoContrato(1);
    detectarPaqueteContrato();
  ...
  function errorMonto(viewId, valor){
    const etiqueta = ETIQUETA_MONTO[viewId] || 'El valor';
  ...
  function marcarErrorCampo(viewId, mensaje){
    const view = $(viewId);
    if (!view) return;
    const caja = view.closest('.form-group') || view.parentNode;
    const previo = caja.querySelector('.field-error');
    if (previo) previo.remove();
    view.classList.toggle('has-error', !!mensaje);
    if (mensaje) {
  ...
  function sincronizarMoneyInput(viewId, hiddenId){
    ...
    function actualizar(formatear){
      const mensaje = errorMonto(viewId, view.value);
      marcarErrorCampo(viewId, mensaje);
      if (!mensaje) {
        const limpio = limpiarNumero(view.value);
        if (formatear) view.value = formatearMiles(limpio);
        hidden.value = limpio;
      } else {
        // Se envía tal cual: si el usuario insiste, el servidor también lo rechaza.
        hidden.value = view.value.trim();
      }
      return mensaje;
    }
    ...
    if (view.value) actualizar(true);
  }

  const campoJefe = $('jefe_inmediato');
  if (campoJefe) {
  ...
  // ── Auxilio de transporte legal (valores en validaciones_contrato.php) ──
  const SALARIO_MINIMO = <?= (int)SALARIO_MINIMO_VIGENTE ?>;
  const AUXILIO_LEGAL = <?= (int)AUXILIO_TRANSPORTE_VIGENTE ?>;
  const TOPE_AUXILIO = <?= (int)(TOPE_SALARIOS_MINIMOS_AUXILIO * SALARIO_MINIMO_VIGENTE) ?>;

  function auxilioQueCorresponde() {
    const salario = Number(limpiarNumero($('salario_base_view')?.value || '')) || 0;
    if (!salario) return null;
    return salario <= TOPE_AUXILIO ? AUXILIO_LEGAL : 0;
  }

  function actualizarAyudaAuxilio() {
    const texto = $('auxilioLegalTexto');
    const caja = $('auxilioLegal');
    if (!texto || !caja) return;
    const legal = auxilioQueCorresponde();
    const actual = Number(limpiarNumero($('auxilio_transporte_view')?.value || '0')) || 0;
    if (legal === null) {
      texto.textContent = 'Valor legal 2026: ' + moneyCOP(AUXILIO_LEGAL) + ' (solo si el salario es hasta ' + moneyCOP(TOPE_AUXILIO) + ', 2 salarios mínimos).';
      caja.classList.remove('no-coincide');
    } else if (legal === 0) {
      texto.textContent = 'Con este salario (más de 2 salarios mínimos) no corresponde auxilio de transporte: debería ser $0.';
      caja.classList.toggle('no-coincide', actual !== 0);
    } else {
      texto.textContent = actual === legal
        ? 'Coincide con el valor legal 2026 (' + moneyCOP(AUXILIO_LEGAL) + ').'
        : 'Por este salario corresponde el auxilio legal 2026: ' + moneyCOP(AUXILIO_LEGAL) + '.';
      caja.classList.toggle('no-coincide', actual !== legal);
    }
    $('btnAuxilioLegal').hidden = legal === null || actual === legal;
  }

  $('btnAuxilioLegal')?.addEventListener('click', function () {
    const legal = auxilioQueCorresponde();
    if (legal === null) return;
    const vista = $('auxilio_transporte_view');
    vista.value = String(legal);
    vista.dispatchEvent(new Event('input'));
    vista.dispatchEvent(new Event('blur'));
    actualizarAyudaAuxilio();
  });
  ['salario_base_view', 'auxilio_transporte_view'].forEach(function (id) {
    $(id)?.addEventListener('input', actualizarAyudaAuxilio);
  });
  actualizarAyudaAuxilio();
  ...
  if (formContrato) {
    formContrato.addEventListener('submit', function(e){
      ...
      // Montos: si alguno tiene letras o símbolos, no se envía (el servidor también lo rechazaría).
      const errorSalario = salarioView && salarioView.validarMonto ? salarioView.validarMonto() : '';
