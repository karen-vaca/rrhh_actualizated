...
try {
    $r = registrarResultadoExamen($conexion, $_POST);
} catch (Throwable $e) {
...
if (!$r['ok']) {
    volverExamenes('No se registró el resultado: ' . reset($r['errores']), 'error');
}
volverExamenes('Resultado registrado. El perfil de salud y el historial del trabajador quedaron actualizados.', 'ok');
