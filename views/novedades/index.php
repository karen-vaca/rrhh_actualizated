...
function firstCharNovedades(string $text): string {
    $text = trim($text);
    if ($text === '') return '';
    return function_exists('mb_substr') ? mb_substr($text, 0, 1, 'UTF-8') : substr($text, 0, 1);
}

function inicialesPersonaNovedades($nombres, $apellidos = ''): string {
    $nombres = trim((string)$nombres);
    $apellidos = trim((string)$apellidos);
    $i1 = $nombres !== '' ? strtoupper(firstCharNovedades($nombres)) : '';
    $i2 = $apellidos !== '' ? strtoupper(firstCharNovedades($apellidos)) : '';

    if ($i2 === '' && $nombres !== '') {
        $partes = preg_split('/\s+/', $nombres);
        $i2 = isset($partes[1]) ? strtoupper(firstCharNovedades($partes[1])) : '';
    }
...
function fmtFechaNovedades($fecha): string {
    if (!$fecha || $fecha === '0000-00-00') return '—';
    $time = strtotime((string)$fecha);
    return $time ? date('d/m/Y', $time) : '—';
}

function tablaExisteNovedades(PDO $conexion, string $tabla): bool {
...
function columnaExisteNovedades(PDO $conexion, string $tabla, string $columna): bool {
...
function baseUrlNovedades(string $path = ''): string {
...
function mensajeTextoNovedades(?string $mensaje): ?string {
    switch ($mensaje) {
        case 'creado': return 'Novedad registrada correctamente.'; 
        case 'actualizado': return 'Novedad actualizada correctamente.';
        case 'aprobada': return 'Novedad aprobada correctamente.';
        ...
        case 'error': return 'Ocurrió un error al procesar la solicitud.';
        default: return null;
    }
}

function badgeNovedad(string $estado): string {
    switch ($estado) {
        case 'Aprobada': return 'badge-ok';
        case 'Pendiente': return 'badge-warn';
        case 'Rechazada': return 'badge-danger';
        case 'Cerrada': return 'badge-muted';
        default: return 'badge-muted';
    }
}

function catClassNovedad(string $categoria): string {
    switch ($categoria) {
        case 'Laboral': return 'cat-blue';
        case 'Asistencia': return 'cat-orange';
        case 'Salud y SST': return 'cat-green';
        case 'Disciplinaria': return 'cat-red';
        default: return 'cat-blue';
    }
}

function avatarClassNovedad($idTrabajador): string {
    $clases = ['avatar-blue', 'avatar-green', 'avatar-orange', 'avatar-purple', 'avatar-teal', 'avatar-red'];
    $id = abs((int)$idTrabajador);
    return $clases[$id % count($clases)];
}

function subirSoporteNovedad(string $campo, ?string &$error): ?string {
    if (!isset($_FILES[$campo]) || !is_array($_FILES[$campo]) || ($_FILES[$campo]['error'] ?? UPLOAD_ERR_NO_FILE) === UPLOAD_ERR_NO_FILE) {
        return null;
    }
    ...
    if ($tamano > 5 * 1024 * 1024) {
        $error = 'archivo_pesado';
        return null;
    }

    $nombreOriginal = (string)($_FILES[$campo]['name'] ?? '');
    ...
    $directorio = __DIR__ . '/../../uploads/novedades/';
    ...
    if (!move_uploaded_file((string)$_FILES[$campo]['tmp_name'], $destino)) {
        $error = 'archivo_invalido';
        return null;
    }
...
$inicialSesion = inicialesPersonaNovedades($nombresSesion, $apellidosSesion);
...
$categorias = ['Laboral', 'Asistencia', 'Salud y SST', 'Disciplinaria'];
$estados = ['Pendiente', 'Aprobada', 'Rechazada', 'Cerrada'];
$tiposNovedad = [
    'Laboral' => ['Cambio de cargo', 'Cambio de área', 'Cambio de horario', 'Llamado de atención', 'Reconocimiento', 'Retiro', 'Otro'],
    'Asistencia' => ['Llegada tarde', 'Ausencia', 'Permiso', 'Horas extra', 'Cambio de turno', 'Otro'],
    'Salud y SST' => ['Incapacidad', 'Recomendación médica', 'Accidente laboral', 'Restricción médica', 'Seguimiento SST', 'Otro'],
    'Disciplinaria' => ['Descargo', 'Suspensión', 'Compromiso laboral', 'Incumplimiento', 'Otro']
];

$mensajeRedireccion = null;

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $accion = $_POST['accion'] ?? '';

    try {
        if (!tablaExisteNovedades($conexion, 'novedades')) {
        ...
        if ($accion === 'crear' || $accion === 'actualizar') {
            ...
            $tipoNovedad = trim((string)($_POST['tipo_novedad'] ?? ''));
            ...
            $responsable = trim((string)($_POST['responsable'] ?? $nombreSesionCompleto));
            $observaciones = trim((string)($_POST['observaciones'] ?? ''));

            if ($idTrabajador <= 0 || $categoria === '' || $tipoNovedad === '' || $fechaInicio === '') {
            ...
            $responsable = $responsable !== '' ? $responsable : $nombreSesionCompleto;

            $errorArchivo = null;
            $archivo = subirSoporteNovedad('soporte_archivo', $errorArchivo);
            if ($errorArchivo !== null) {
                header('Location: index.php?mensaje=' . urlencode($errorArchivo));
                exit;
            }

            if ($accion === 'crear') {
                $stmt = $conexion->prepare("\n                    INSERT INTO novedades\n                    (id_trabajador, categoria, tipo_novedad, fecha_inicio, fecha_fin, descripcion, estado, responsable, soporte_archivo, observaciones)\n                    VALUES\n                    (:id_trabajador, :categoria, :tipo_novedad, :fecha_inicio, :fecha_fin, :descripcion, :estado, :responsable, :soporte_archivo, :observaciones)\n                ");
                $stmt->execute([
            ...
            if ($accion === 'actualizar') {
                ...
                if ($archivo !== null) {
                    $sql .= ", soporte_archivo = :soporte_archivo";
                    $params[':soporte_archivo'] = $archivo;
                }

                $sql .= " WHERE id_novedad = :id_novedad LIMIT 1";
        ...
        if ($accion === 'cambiar_estado') {
            ...
            if ($idNovedad <= 0 || !in_array($estado, $estados, true)) {
                header('Location: index.php?mensaje=id_invalido');
                exit;
            }

            $stmt = $conexion->prepare("UPDATE novedades SET estado = :estado WHERE id_novedad = :id LIMIT 1");
            $stmt->execute([':estado' => $estado, ':id' => $idNovedad]);

            $msg = 'actualizado';
            if ($estado === 'Aprobada') $msg = 'aprobada';
            if ($estado === 'Rechazada') $msg = 'rechazada';
            if ($estado === 'Cerrada') $msg = 'cerrada';

            header('Location: index.php?mensaje=' . $msg);
            exit;
        }
    } catch (Throwable $e) {
        header('Location: index.php?mensaje=error');
...
$mensajeTexto = mensajeTextoNovedades($mensaje);
$errorCarga = null;
$novedades = [];
$trabajadores = [];
...
try {
    $estadoTrabajadorSql = columnaExisteNovedades($conexion, 'trabajadores', 'estado')
        ? 'COALESCE(t.estado, 1)'
        : '1';

    if (!tablaExisteNovedades($conexion, 'novedades')) {
        $errorCarga = 'Falta crear la tabla novedades en la base de datos.';
    } else {
        $sqlNovedades = "\n            SELECT\n                n.id_novedad,\n                n.id_trabajador,\n                n.categoria,\n                n.tipo_novedad,\n                n.fecha_inicio,\n                n.fecha_fin,\n                n.descripcion,\n                n.estado,\n                n.responsable,\n                n.soporte_archivo,\n                n.observaciones,\n                n.created_at,\n                n.updated_at,\n                t.nombres,\n                t.apellidos,\n                t.numero_documento,\n                {$estadoTrabajadorSql} AS estado_trabajador,\n                COALESCE(a.nombre_area, 'Sin área') AS nombre_area,\n                COALESCE(c.nombre_cargo, 'Sin cargo') AS nombre_cargo\n            FROM novedades n\n            LEFT JOIN trabajadores t ON n.id_trabajador = t.id_trabajador\n            LEFT JOIN (\n                SELECT dl1.*\n                FROM datos_laborales dl1\n                INNER JOIN (\n                    SELECT id_trabajador, MAX(id_datos_laborales) AS max_id\n                    FROM datos_laborales\n                    GROUP BY id_trabajador\n                ) ult ON ult.max_id = dl1.id_datos_laborales\n            ) dl ON dl.id_trabajador = t.id_trabajador\n            LEFT JOIN areas a ON dl.id_areas = a.id_areas\n            LEFT JOIN cargos c ON dl.id_cargo = c.id_cargo\n            ORDER BY n.fecha_inicio DESC, n.id_novedad DESC\n        ";
        $novedades = $conexion->query($sqlNovedades)->fetchAll();

        foreach ($novedades as &$novedad) {
            $nombresT = $novedad['nombres'] ?? '';
            $apellidosT = $novedad['apellidos'] ?? '';
            $nombreCompleto = trim($nombresT . ' ' . $apellidosT);
            $soporte = trim((string)($novedad['soporte_archivo'] ?? ''));

            $novedad['id'] = (int)($novedad['id_novedad'] ?? 0);
            $novedad['trabajador_id'] = (int)($novedad['id_trabajador'] ?? 0);
            $novedad['trabajador_nombre'] = $nombreCompleto !== '' ? $nombreCompleto : 'Trabajador no encontrado';
            $novedad['iniciales'] = inicialesPersonaNovedades($nombresT, $apellidosT);
            $novedad['avatar_class'] = avatarClassNovedad($novedad['id_trabajador'] ?? 0);
            $novedad['area'] = $novedad['nombre_area'] ?? 'Sin área';
            $novedad['cargo'] = $novedad['nombre_cargo'] ?? 'Sin cargo';
            $novedad['documento'] = $novedad['numero_documento'] ?? '';
            $novedad['estado_trabajador_texto'] = ((int)($novedad['estado_trabajador'] ?? 1) === 0) ? 'Inactivo' : 'Activo';
            $novedad['soporte_url'] = $soporte !== '' ? baseUrlNovedades('uploads/novedades/' . $soporte) : '';
        }
        unset($novedad);
    }

    $sqlTrabajadores = "\n        SELECT\n            t.id_trabajador,\n            t.nombres,\n            t.apellidos,\n            t.numero_documento,\n            {$estadoTrabajadorSql} AS estado,\n            COALESCE(a.nombre_area, 'Sin área') AS nombre_area,\n            COALESCE(c.nombre_cargo, 'Sin cargo') AS nombre_cargo\n        FROM trabajadores t\n        LEFT JOIN (\n            SELECT dl1.*\n            FROM datos_laborales dl1\n            INNER JOIN (\n                SELECT id_trabajador, MAX(id_datos_laborales) AS max_id\n                FROM datos_laborales\n                GROUP BY id_trabajador\n            ) ult ON ult.max_id = dl1.id_datos_laborales\n        ) dl ON dl.id_trabajador = t.id_trabajador\n        LEFT JOIN areas a ON dl.id_areas = a.id_areas\n        LEFT JOIN cargos c ON dl.id_cargo = c.id_cargo\n        WHERE {$estadoTrabajadorSql} = 1\n        ORDER BY t.nombres ASC, t.apellidos ASC\n    ";
    $trabajadores = $conexion->query($sqlTrabajadores)->fetchAll();
    $trabajadoresActivos = count($trabajadores);
} catch (Throwable $e) {
    $errorCarga = 'Error al cargar novedades: ' . $e->getMessage();
}
...
foreach ($novedades as $n) {
    if (($n['estado'] ?? '') === 'Pendiente') $stats['pendientes']++;
    if (($n['estado'] ?? '') === 'Aprobada') $stats['aprobadas']++;
    if (($n['estado'] ?? '') === 'Rechazada') $stats['rechazadas']++;
    if (($n['estado'] ?? '') === 'Cerrada') $stats['cerradas']++;
    if (($n['categoria'] ?? '') === 'Salud y SST') $stats['sst']++;
}

$novedadesJson = json_encode($novedades, JSON_UNESCAPED_UNICODE | JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP);
if ($novedadesJson === false) $novedadesJson = '[]';

$tiposJson = json_encode($tiposNovedad, JSON_UNESCAPED_UNICODE | JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP);
if ($tiposJson === false) $tiposJson = '{}';
?>
<!DOCTYPE html>
<html lang="es">
<head>
...
.page-header{display:flex;align-items:center;justify-content:space-between;gap:16px;flex-wrap:wrap;margin-bottom:22px}
...
.btn-new svg{width:16px;height:16px;stroke:#021a08;fill:none;stroke-width:2.5;stroke-linecap:round;stroke-linejoin:round}
.hero{background:linear-gradient(135deg,#052817,#0b3a20 55%,#0f8a3b);border-radius:20px;padding:24px 28px;color:#fff;box-shadow:var(--shadow);display:flex;justify-content:space-between;gap:20px;align-items:center;margin-bottom:20px;overflow:hidden;position:relative}
.hero:after{content:"";position:absolute;right:-90px;top:-90px;width:240px;height:240px;border-radius:50%;background:rgba(255,255,255,.08)}
.hero h1{font-family:'Syne',sans-serif;font-size:32px;font-weight:800;line-height:1;letter-spacing:-1px;margin-bottom:10px;position:relative;z-index:1}
.hero p{color:rgba(255,255,255,.72);font-size:13.5px;font-weight:400;max-width:720px;position:relative;z-index:1;line-height:1.35}
.hero-actions{display:flex;gap:10px;position:relative;z-index:1}
.primary-btn,.secondary-btn,.danger-btn,.small-btn,.clear-btn,.save-btn,.btn-filter{border:none;border-radius:12px;font-weight:600;display:inline-flex;align-items:center;justify-content:center;gap:8px;transition:.15s}
.primary-btn{height:42px;padding:0 16px;background:#fff;color:#073f1e;box-shadow:0 12px 24px rgba(0,0,0,.10);font-family:'Syne',sans-serif;font-weight:700;font-size:13px}
.primary-btn:hover{transform:translateY(-1px)}
.secondary-btn{height:42px;padding:0 15px;background:#eef8f1;color:#0f7a38}
.danger-btn{height:35px;padding:0 10px;background:#fff1f2;color:#e11d48;font-size:12px}
.small-btn{height:35px;padding:0 10px;background:#eef8f1;color:#0f7a38;font-size:12px}
.clear-btn{height:42px;padding:0 14px;background:#102016;color:#fff}
.save-btn{height:42px;padding:0 16px;background:var(--green);color:#062511}
...
/* ── FILTERS BAR patrón Trabajadores ── */
.filters{background:var(--white);border:1px solid var(--border);border-radius:14px;padding:14px 18px;display:flex;align-items:flex-end;gap:12px;flex-wrap:wrap;box-shadow:var(--shadow);margin-bottom:16px}
.filter-group{display:flex;flex-direction:column;gap:4px;min-width:150px}
.filter-group.search-group{flex:1;min-width:320px}
.filter-label{font-size:10.5px;font-weight:600;color:var(--text-soft);text-transform:uppercase;letter-spacing:.5px}
.search-wrap{height:40px;display:flex;align-items:center;gap:8px;background:var(--white);border:1px solid var(--border);border-radius:10px;padding:0 14px;transition:border-color .2s,box-shadow .2s}
.search-wrap:focus-within{border-color:#b6dfc4;box-shadow:0 0 0 3px rgba(45,223,110,0.07)}
.search-wrap svg{width:15px;height:15px;stroke:var(--text-soft);fill:none;stroke-width:1.8;flex-shrink:0;stroke-linecap:round;stroke-linejoin:round}
.search-wrap input{background:none;border:none;outline:none;font-size:13.5px;color:var(--text-mid);font-family:'DM Sans',sans-serif;width:100%;font-weight:400}
...
.btn-filter:hover{border-color:#b6dfc4;background:#f7fbf8;color:var(--green-dark)}
.clear-btn{height:40px;padding:0 16px;background:var(--white);border:1px solid #fecaca;color:#dc2626;border-radius:10px;font-size:13px;font-weight:500;font-family:'DM Sans',sans-serif}
.clear-btn:hover{background:#fff1f2;border-color:#fca5a5}

.card{background:var(--white);border:1px solid var(--border);border-radius:16px;box-shadow:var(--shadow);overflow:hidden}.table-head{display:flex;align-items:center;justify-content:space-between;padding:14px 20px;border-bottom:1px solid var(--border);gap:12px}.table-title{font-family:'DM Sans',sans-serif;font-size:13px;font-weight:400;color:var(--text-soft)}.table-title strong{color:var(--text);font-weight:600}.table-actions{display:flex;align-items:center;gap:8px}.table-wrap{overflow:auto}table{width:100%;border-collapse:collapse;min-width:1040px}th{font-size:11px;text-transform:uppercase;letter-spacing:.8px;color:var(--text-soft);text-align:left;padding:13px 16px;background:#f8fbf9;font-weight:700}td{padding:14px 16px;border-top:1px solid #edf4ef;vertical-align:middle;font-weight:400;font-size:13.5px;color:var(--text)}tbody tr:hover{background:#fbfdfc}.person{display:flex;align-items:center;gap:11px}.avatar{width:38px;height:38px;border-radius:50%;color:#fff;display:flex;align-items:center;justify-content:center;font-family:'Syne',sans-serif;font-weight:800;font-size:13px;flex-shrink:0;box-shadow:0 2px 8px rgba(0,0,0,.12)}.avatar-blue{background:linear-gradient(135deg,#3b82f6,#1d4ed8)}.avatar-green{background:linear-gradient(135deg,#22c55e,#15803d)}.avatar-orange{background:linear-gradient(135deg,#f59e0b,#d97706)}.avatar-purple{background:linear-gradient(135deg,#8b5cf6,#6d28d9)}.avatar-teal{background:linear-gradient(135deg,#14b8a6,#0f766e)}.avatar-red{background:linear-gradient(135deg,#ef4444,#b91c1c)}.cell-main{font-weight:600;color:var(--text);font-size:13.5px}.cell-sub{font-size:11.5px;color:var(--text-soft);font-weight:500;margin-top:3px}.badge,.cat-tag{display:inline-flex;align-items:center;border-radius:999px;font-size:11.5px;font-weight:600;padding:6px 10px;white-space:nowrap}.badge-ok{background:#dcfce7;color:#166534}.badge-warn{background:#fef3c7;color:#92400e}.badge-danger{background:#ffe4e6;color:#be123c}.badge-muted{background:#e2e8f0;color:#475569}.cat-blue{background:#dbeafe;color:#1d4ed8}.cat-orange{background:#ffedd5;color:#c2410c}.cat-green{background:#dcfce7;color:#166534}.cat-red{background:#ffe4e6;color:#be123c}.actions{display:flex;gap:6px;align-items:center;flex-wrap:nowrap}.inline{display:inline-flex}.icon-btn{width:32px;height:32px;border-radius:8px;background:var(--white);border:1px solid transparent;display:inline-flex;align-items:center;justify-content:center;cursor:pointer;transition:all .18s;color:var(--text-soft);text-decoration:none;padding:0;font-family:'DM Sans',sans-serif}.icon-btn svg{width:14px;height:14px;stroke:currentColor;fill:none;stroke-width:1.8;stroke-linecap:round;stroke-linejoin:round}.icon-btn:hover{background:#f7fbf8;border-color:var(--border);color:var(--green-dark)}.icon-btn.ok:hover{background:#f0fdf4;border-color:#bbf7d0;color:#16a34a}.icon-btn.danger:hover{background:#fff1f2;border-color:#fecaca;color:#dc2626}.icon-btn.support{border-color:var(--border);background:#fff}.empty{padding:42px 20px;text-align:center;color:#789184}.empty strong{display:block;font-family:'Syne',sans-serif;font-size:22px;color:#102016;margin-bottom:6px;font-weight:700}
...
.novedades-page .stat-sub,
.novedades-page .cell-sub,
.novedades-page .descripcion-box{font-weight:400}
.novedades-page .search-wrap input,
.novedades-page .filter-input,
.novedades-page .filter-select,
.novedades-page .form-input,
.novedades-page .form-select,
.novedades-page .form-textarea{font-weight:400}
.novedades-page .cell-main{font-weight:600}
.novedades-page .badge,
...
<?php if ($errorCarga): ?>
  <div class="toast error" id="toastSistema">
    <strong>Revisión necesaria</strong>
    <span><?= e($errorCarga) ?></span>
  </div>
<?php endif; ?>
...
<main class="main">
  ...
  <section class="page-header">
    <div class="page-header-left">
      ...
      <div>
        ...
        <p class="page-sub">Administra novedades laborales, asistencias, casos disciplinarios y eventos SG-SST.</p>
      </div>
    </div>
    <div class="page-header-right">
      <a class="btn-back" href="../dashboard/dashboard.php">
        <svg viewBox="0 0 24 24"><path d="M15 18l-6-6 6-6"/></svg>
        Volver al Panel
      </a>
      <button class="btn-new" type="button" onclick="abrirModalCrear()">
  ...
  <section class="stats">
    <div class="stat"><div class="stat-label">Total</div><div class="stat-num"><?= (int)$stats['total'] ?></div><div class="stat-sub">Novedades</div></div>
    <div class="stat orange"><div class="stat-label">Pendientes</div><div class="stat-num"><?= (int)$stats['pendientes'] ?></div><div class="stat-sub">Por revisar</div></div>
    <div class="stat green"><div class="stat-label">Aprobadas</div><div class="stat-num"><?= (int)$stats['aprobadas'] ?></div><div class="stat-sub">Gestionadas</div></div>
    <div class="stat gray"><div class="stat-label">Cerradas</div><div class="stat-num"><?= (int)$stats['cerradas'] ?></div><div class="stat-sub">Finalizadas</div></div>
    <div class="stat teal"><div class="stat-label">Trabajadores</div><div class="stat-num"><?= (int)$trabajadoresActivos ?></div><div class="stat-sub">Activos</div></div>
  </section>
  ...
  <section class="filters">
    <div class="filter-group search-group">
      ...
      <div class="search-wrap">
        ...
        <input type="text" id="busqueda" placeholder="Buscar por trabajador, documento o tipo..." oninput="if($('busquedaTop')) $('busquedaTop').value=this.value; filtrarNovedades()">
      </div>
    </div>

    <div class="filter-group">
      <label class="filter-label" for="filtroEstado">Estado</label>
      <select class="filter-select" id="filtroEstado" onchange="filtrarNovedades()">
    ...
    <div class="filter-group">
      <label class="filter-label" for="filtroCategoria">Categoría</label>
      <select class="filter-select" id="filtroCategoria" onchange="filtrarNovedades()">
    ...
    <div class="filter-group">
      <label class="filter-label" for="filtroDesde">Desde</label>
      <input class="filter-input" type="date" id="filtroDesde" onchange="filtrarNovedades()">
    </div>

    <div class="filter-group">
      <label class="filter-label" for="filtroHasta">Hasta</label>
      <input class="filter-input" type="date" id="filtroHasta" onchange="filtrarNovedades()">
    </div>

    <button class="btn-filter" type="button" onclick="filtrarNovedades()">
      <svg viewBox="0 0 24 24"><path d="M22 3H2l8 9.46V19l4 2v-8.54L22 3z"/></svg>
      Filtrar
    </button>
    <button class="clear-btn" type="button" onclick="limpiarFiltros()">× Limpiar</button>
  </section>

  <section class="card">
    <div class="table-head">
      <div class="table-title">Mostrando <strong id="contadorVisibles"><?= count($novedades) ?></strong> de <strong><?= count($novedades) ?></strong> novedades</div>
      <div class="table-actions">
        <button class="icon-btn support" type="button" title="Exportar listado" onclick="window.print()">
        ...
        <button class="icon-btn support" type="button" title="Imprimir listado" onclick="window.print()">
    ...
    <?php if (count($novedades) === 0): ?>
      <div class="empty">
        ...
        <span>Cuando registres una novedad aparecerá en este panel.</span>
      </div>
    <?php else: ?>
      <div class="table-wrap">
        <table id="tablaNovedades">
          ...
          <tbody>
          <?php foreach ($novedades as $idx => $n): ?>
            <tr
              data-categoria="<?= e($n['categoria'] ?? '') ?>"
              data-estado="<?= e($n['estado'] ?? '') ?>"
              data-fecha="<?= e($n['fecha_inicio'] ?? '') ?>"
              data-texto="<?= e(strtolower(($n['trabajador_nombre'] ?? '') . ' ' . ($n['documento'] ?? '') . ' ' . ($n['tipo_novedad'] ?? '') . ' ' . ($n['categoria'] ?? '') . ' ' . ($n['estado'] ?? ''))) ?>"
            >
              <td>
                <div class="person">
                  <div class="avatar <?= e($n['avatar_class'] ?? 'avatar-blue') ?>"><?= e($n['iniciales'] ?? 'TR') ?></div>
                  <div>
                    <div class="cell-main"><?= e($n['trabajador_nombre'] ?? 'Trabajador') ?></div>
                    <div class="cell-sub"><?= e($n['documento'] ?? '') ?> · <?= e($n['area'] ?? 'Sin área') ?></div>
                  </div>
                </div>
              </td>
              <td>
                <span class="cat-tag <?= e(catClassNovedad($n['categoria'] ?? '')) ?>"><?= e($n['categoria'] ?? '') ?></span>
                <div class="cell-sub"><?= e($n['tipo_novedad'] ?? '') ?></div>
              </td>
              <td>
                <div class="cell-main"><?= e(fmtFechaNovedades($n['fecha_inicio'] ?? null)) ?></div>
                <div class="cell-sub">Fin: <?= e(fmtFechaNovedades($n['fecha_fin'] ?? null)) ?></div>
              </td>
              <td><span class="badge <?= e(badgeNovedad($n['estado'] ?? '')) ?>"><?= e($n['estado'] ?? '') ?></span></td>
              <td>
                <div class="cell-main"><?= e($n['responsable'] ?? '—') ?></div>
                <div class="cell-sub"><?= e($n['estado_trabajador_texto'] ?? 'Activo') ?></div>
              </td>
              <td>
                <?php if (!empty($n['soporte_url'])): ?>
                  <a class="icon-btn support" href="<?= e($n['soporte_url']) ?>" target="_blank" title="Ver soporte">
                ...
                <?php else: ?>
                  <span class="cell-sub">Sin soporte</span>
                <?php endif; ?>
              </td>
              <td>
                <div class="actions">
                  <button class="icon-btn" type="button" onclick="verNovedad(<?= (int)$idx ?>)" title="Ver novedad">
                    <svg viewBox="0 0 24 24"><path d="M1 12s4-7 11-7 11 7 11 7-4 7-11 7-11-7-11-7z"/><circle cx="12" cy="12" r="3"/></svg>
                  </button>
                  <button class="icon-btn" type="button" onclick="editarNovedad(<?= (int)$idx ?>)" title="Editar novedad">
                  ...
                  <?php if (($n['estado'] ?? '') !== 'Aprobada'): ?>
                    <form class="inline" method="POST"><?php echo campoCsrf(); ?>
                      ...
                      <button class="icon-btn ok" type="submit" title="Aprobar novedad">
                  ...
                  <?php if (($n['estado'] ?? '') !== 'Cerrada'): ?>
                    <form class="inline" method="POST"><?php echo campoCsrf(); ?>
                      ...
                      <button class="icon-btn danger" type="submit" title="Cerrar novedad">
...
<div class="overlay" id="modalFormulario">
  <div class="modal">
    ...
    <form method="POST" enctype="multipart/form-data" id="formNovedad"><?php echo campoCsrf(); ?>
      ...
      <div class="modal-body">
        <div class="form-group">
          ...
          <select class="form-select" name="id_trabajador" id="idTrabajador" required>
            <option value="">Selecciona un trabajador</option>
            <?php foreach ($trabajadores as $t):
              $nombreT = trim(($t['nombres'] ?? '') . ' ' . ($t['apellidos'] ?? ''));
            ?>
              <option value="<?= (int)$t['id_trabajador'] ?>">
                <?= e($nombreT) ?> · <?= e($t['numero_documento'] ?? '') ?> · <?= e($t['nombre_area'] ?? 'Sin área') ?>
              </option>
            <?php endforeach; ?>
          </select>
        </div>
        ...
        <div class="form-grid">
          <div class="form-group">
            ...
            <select class="form-select" name="categoria" id="categoria" onchange="cargarTipos()" required>
              <?php foreach ($categorias as $categoria): ?>
                <option value="<?= e($categoria) ?>"><?= e($categoria) ?></option>
              <?php endforeach; ?>
            </select>
          ...
          <div class="form-group">
            ...
            <select class="form-select" name="tipo_novedad" id="tipoNovedad" required></select>
          </div>
        </div>
        ...
        <div class="form-grid g3">
          <div class="form-group">
            ...
            <input class="form-input" type="date" name="fecha_inicio" id="fechaInicio" required>
          </div>
          <div class="form-group">
        ...
        <div class="form-group">
          ...
          <input class="form-input" type="text" name="responsable" id="responsable" value="<?= e($nombreSesionCompleto) ?>">
        </div>

        <div class="form-group">
          <label class="form-label">Descripción</label>
          <textarea class="form-textarea" name="descripcion" id="descripcion" placeholder="Describe la novedad..."></textarea>
        </div>

        <div class="form-group">
          <label class="form-label">Observaciones</label>
          <textarea class="form-textarea" name="observaciones" id="observaciones" placeholder="Notas internas o seguimiento..."></textarea>
        </div>

        <div class="form-group">
          <label class="form-label">Soporte PDF/JPG/PNG, máximo 5 MB</label>
          <input class="form-input" type="file" name="soporte_archivo" accept=".pdf,.jpg,.jpeg,.png">
        </div>
      </div>
      <div class="modal-footer">
        <button class="secondary-btn" type="button" onclick="cerrarModalFormulario()">Cancelar</button>
...
<div class="overlay" id="modalDetalle">
  <div class="modal">
    <div class="modal-head">
      <div class="modal-title">Detalle de novedad</div>
      <button class="modal-close" type="button" onclick="cerrarModalDetalle()">×</button>
    </div>
    <div class="modal-body" id="detalleContenido"></div>
    <div class="modal-footer">
      <button class="secondary-btn" type="button" onclick="cerrarModalDetalle()">Cerrar</button>
    </div>
  </div>
</div>
...
const tiposPorCategoria = <?= $tiposJson ?>;
let tabActual = '';
...
document.addEventListener('keydown', function(e){
  if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === 'k') {
    e.preventDefault();
    const topSearch = $('busquedaTop') || $('busqueda');
...
function fmtFechaJS(fecha){
  if (!fecha || fecha === '0000-00-00') return '—';
  const p = String(fecha).split('-');
  return p.length === 3 ? p[2] + '/' + p[1] + '/' + p[0] : fecha;
}
function badgeClassJS(estado){
  if (estado === 'Aprobada') return 'badge-ok';
  if (estado === 'Pendiente') return 'badge-warn';
  if (estado === 'Rechazada') return 'badge-danger';
  if (estado === 'Cerrada') return 'badge-muted';
  return 'badge-muted';
}
function catClassJS(categoria){
  if (categoria === 'Laboral') return 'cat-blue';
  if (categoria === 'Asistencia') return 'cat-orange';
  if (categoria === 'Salud y SST') return 'cat-green';
  if (categoria === 'Disciplinaria') return 'cat-red';
  return 'cat-blue';
}
function sincronizarBusqueda(valor){
  if ($('busqueda')) $('busqueda').value = valor;
  filtrarNovedades();
}
function cambiarTabNovedad(btn){
...
function filtrarNovedades(){
  ...
  let visibles = 0;
  document.querySelectorAll('#tablaNovedades tbody tr').forEach(row => {
    ...
    if (estado && rowEstado !== estado) ok = false;
    if (categoria && rowCategoria !== categoria) ok = false;
    if (desde && fecha && fecha < desde) ok = false;
    if (hasta && fecha && fecha > hasta) ok = false;

    row.style.display = ok ? '' : 'none';
    if (ok) visibles++;
  });
  const contador = $('contadorVisibles');
  if (contador) contador.textContent = visibles;
}
function limpiarFiltros(){
  $('busqueda').value = '';
  if ($('busquedaTop')) $('busquedaTop').value = '';
  $('filtroEstado').value = '';
...
function cargarTipos(valorSeleccionado = ''){
  const categoria = $('categoria').value;
  const select = $('tipoNovedad');
  select.innerHTML = '';
  ...
  tipos.forEach(tipo => {
    ...
    op.value = tipo;
    op.textContent = tipo;
    if (tipo === valorSeleccionado) op.selected = true;
    select.appendChild(op);
  });
}
function abrirModalCrear(){
  $('formTitulo').textContent = 'Registrar novedad';
  $('accionForm').value = 'crear';
  $('idNovedad').value = '';
  $('formNovedad').reset();
  $('responsable').value = <?= json_encode($nombreSesionCompleto, JSON_UNESCAPED_UNICODE) ?>;
  $('estado').value = 'Pendiente';
  cargarTipos();
  $('modalFormulario').classList.add('show');
}
function editarNovedad(idx){
  const n = novedades[idx];
  if (!n) return;
  $('formTitulo').textContent = 'Editar novedad';
  $('accionForm').value = 'actualizar';
  $('idNovedad').value = n.id || n.id_novedad || '';
  $('idTrabajador').value = n.id_trabajador || '';
  $('categoria').value = n.categoria || 'Laboral';
  cargarTipos(n.tipo_novedad || '');
  $('fechaInicio').value = n.fecha_inicio || '';
  $('fechaFin').value = n.fecha_fin || '';
  $('estado').value = n.estado || 'Pendiente';
  $('responsable').value = n.responsable || '';
  $('descripcion').value = n.descripcion || '';
  $('observaciones').value = n.observaciones || '';
  $('modalFormulario').classList.add('show');
}
function cerrarModalFormulario(){ $('modalFormulario').classList.remove('show'); }
function verNovedad(idx){
  const n = novedades[idx];
  if (!n) return;
  $('detalleContenido').innerHTML = `
    <div class="person">
      <div class="avatar ${escapeHtml(n.avatar_class || 'avatar-blue')}">${escapeHtml(n.iniciales || 'TR')}</div>
      <div>
        <div class="cell-main">${escapeHtml(n.trabajador_nombre || 'Trabajador')}</div>
        <div class="cell-sub">${escapeHtml(n.documento || '')} · ${escapeHtml(n.area || 'Sin área')} · ${escapeHtml(n.cargo || 'Sin cargo')}</div>
      </div>
    </div>
    <div class="detail-grid">
      <div class="detail-item"><div class="detail-label">Categoría</div><div class="detail-value"><span class="cat-tag ${catClassJS(n.categoria || '')}">${escapeHtml(n.categoria || '')}</span></div></div>
      <div class="detail-item"><div class="detail-label">Tipo</div><div class="detail-value">${escapeHtml(n.tipo_novedad || '')}</div></div>
      <div class="detail-item"><div class="detail-label">Fecha inicio</div><div class="detail-value">${fmtFechaJS(n.fecha_inicio)}</div></div>
      <div class="detail-item"><div class="detail-label">Fecha fin</div><div class="detail-value">${fmtFechaJS(n.fecha_fin)}</div></div>
      <div class="detail-item"><div class="detail-label">Estado</div><div class="detail-value"><span class="badge ${badgeClassJS(n.estado || '')}">${escapeHtml(n.estado || '')}</span></div></div>
      <div class="detail-item"><div class="detail-label">Responsable</div><div class="detail-value">${escapeHtml(n.responsable || '—')}</div></div>
    </div>
    <div class="descripcion-box"><strong>Descripción</strong><br>${escapeHtml(n.descripcion || 'Sin descripción.')}</div>
    <div class="descripcion-box"><strong>Observaciones</strong><br>${escapeHtml(n.observaciones || 'Sin observaciones.')}</div>
    ${n.soporte_url ? `<div style="margin-top:13px"><a class="small-btn" href="${escapeHtml(n.soporte_url)}" target="_blank">Abrir soporte</a></div>` : ''}
  `;
  $('modalDetalle').classList.add('show');
}
function cerrarModalDetalle(){ $('modalDetalle').classList.remove('show'); }
